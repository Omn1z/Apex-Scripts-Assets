...
void function Control_RegisterNetworking()
{
	...
	Remote_RegisterServerFunction( "ClientCallback_Control_ProcessRespawnChoice_Homebase" )                                                                                                                                
	...
	if ( IsValid( spawnWaypoint ) )
	{
		...
		switch( waypointType )
		{
			...
			case CONTROL_WAYPOINT_MRB_INDEX:
				...
				break
			default:
				Warning( "Control: Running Control_GetRespawnChoiceFromSpawnWaypoint with an Invalid waypoint type: %i, setting respawn choice to homebase by default", waypointType )
				respawnChoice = eControlPlayerRespawnChoice.SPAWN_AT_BASE
				break
		}
	}
...
entity function Control_GetEntityToSpawnOnFromRespawnChoice( entity wp, int respawnChoice )
{
	...
	{
		printt( "CONTROL: Control_GetEntityToSpawnOnFromRespawnChoice called with an invalid wp" )
	}
...
void function Control_OnPlayerSentSpawnRequest( entity player, entity wp )
{
	...
	Control_PrintSpawningDebug( player, respawnChoice, entityToSpawnOn, "Control_OnPlayerSentSpawnRequest Sending spawn request" )
	...
	if ( Waypoint_GetPingTypeForWaypoint( wp ) == ePingType.NON_PINGABLE_SPAWN_LOCATION )
	{
		...
		switch( waypointType )
		{
			...
			case CONTROL_WAYPOINT_PLAYER_INDEX:
				...
				break
			default:
				Warning( "CONTROL: Got unsupported waypointType in switch statement doing initial settings in Control_ManageRespawnWaypoint_Thread" )
				break
		}
	...
	while ( true )
	{
		...
		switch( waypointType )
		{
			...
			case CONTROL_WAYPOINT_PLAYER_INDEX:
				...
				break
			default:
				Warning( "CONTROL: Got unsupported waypointType in switch statement in loop of Control_ManageRespawnWaypoint_Thread" )
				break
		}
	}
}
...
void function Control_SendRespawnChoiceToServer( int respawnChoice, entity entityToSpawnOn )
{
	...
	Control_PrintSpawningDebug( localPlayer, respawnChoice, entityToSpawnOn, "Control_SendRespawnChoiceToServer called with localPlayer" )
	if ( !IsValid( localPlayer ) || !IsValid( localViewPlayer ) )
	{
		Control_PrintSpawningDebug( localPlayer, respawnChoice, entityToSpawnOn, "Control_SendRespawnChoiceToServer breaking out because localPlayer or localViewPlayer is Not Valid" )
		return
	}
	...
	if ( !localPlayer.GetPlayerNetBool( "control_IsPlayerOnSpawnSelectScreen" ) )
	{
		Control_PrintSpawningDebug( localPlayer, respawnChoice, entityToSpawnOn, "Control_SendRespawnChoiceToServer breaking out because control_IsPlayerOnSpawnSelectScreen is set to false" )
		return
	}

	                                                                                                                          
	if ( respawnChoice == eControlPlayerRespawnChoice.SPAWN_AT_BASE )
		Remote_ServerCallFunction( "ClientCallback_Control_ProcessRespawnChoice_Homebase" )
	else
...
void function ProcessSpawnMenu( entity player )
{
	...
	while ( !IsAlive( player ) )
	{
		...
		foreach ( wp in file.spawnWaypoints )
		{
			...
			if ( shouldShowWaypoints && IsValid( wp ) )
			{
				...
				if ( IsValid( wpParentEnt ) )
				{
					...
					switch( waypointType )
					{
						...
						case CONTROL_WAYPOINT_POINT_INDEX:             
							...
							shouldShowThisWaypoint = true
							int waypointOwner = wpParentEnt.GetWaypointInt( CONTROL_INT_OBJ_TEAM_OWNER )
							bool isSpawnUsable = bool( wpParentEnt.GetWaypointBitfield() ) && waypointOwner != ALLIANCE_NONE

							                                                                                                                
							if ( isSpawnUsable )
								spawnWaypointTeamUsability = waypointOwner == localPlayerAlliance ? eControlSpawnWaypointUsage.FRIENDLY_TEAM : eControlSpawnWaypointUsage.ENEMY_TEAM
							break
						case CONTROL_WAYPOINT_MRB_INDEX:             
							...
							break
						default:
							printt( "Control: Unexpected waypoint type: " + waypointType + " in ProcessSpawnMenu switch statement" )
							break
					}
				}
			}
...
void function UICallback_Control_SpawnButtonClicked( int waypointEHI )
{
	...
	if ( !IsValid( waypoint ) )
		printt( "CONTROL: UICallback_Control_SpawnButtonClicked called with an invalid waypoint" )
...
void function Control_PrintSpawningDebug( entity player, int respawnChoice, entity entityToSpawnOn, string message )
{
	...
	string entToSpawnOnString = IsValid( entityToSpawnOn ) ? string( entityToSpawnOn ) : "EntityToSpawnOnInvalid"

	printt( "CONTROL: " + message + " for player: " + playerEntString + " respawnChoice: " + respawnChoice + " entityToSpawnOn: " + entToSpawnOnString )
}
#endif                    
